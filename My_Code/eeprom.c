...
u16 STMFLASH_BUF_HALFWORD[STM_SECTOR_SIZE/2];    //缓存数组
u32 STMFLASH_BUF_WORD[STM_SECTOR_SIZE/4];    //缓存数组
...
void STMFLASH_Write_HalfWord(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
	...
	while(1) 
	{	
		STMFLASH_Read_HalfWord(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF_HALFWORD,STM_SECTOR_SIZE/2);//读出整个扇区的内容
		for(i=0;i<secremain;i++)	//校验数据
		{
			if(STMFLASH_BUF_HALFWORD[secoff+i]!=0XFFFF)break;//需要擦除  	  
		}
		if(i<secremain)				//需要擦除
		{
			...
			STMFLASH_Write_NoCheck_HalfWord(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF_HALFWORD,STM_SECTOR_SIZE/2);//写入整个扇区  
		}else 
		{
			FLASH_WaitForLastOperation(FLASH_WAITETIME);       	//等待上次操作完成
			STMFLASH_Write_NoCheck_HalfWord(WriteAddr,pBuffer,secremain);//写已经擦除了的,直接写入扇区剩余区间. 
		}
...
/**********************************************************************************
  * 函数功能:从指定地址开始写入指定长度的数据
  * 输入参数:WriteAddr:起始地址(此地址必须为2的倍数!!)、pBuffer:数据指针、NumToWrite：半字(16位)数(就是要写入的32位数据的个数.)
  * 返 回 值: 无
  * 说    明： 
  */
void STMFLASH_Write_Word(u32 WriteAddr,u32 *pBuffer,u16 NumToWrite)	
{
	...
	while(1) 
	{	
		STMFLASH_Read_Word(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF_WORD,STM_SECTOR_SIZE/4);//读出整个扇区的内容(读到BUF中)
		for(i=0;i<secremain;i++)	//校验数据
		{
			if(STMFLASH_BUF_WORD[secoff+i]!=0XFFFF)break;//需要擦除  	  
		}
		if(i<secremain)				//需要擦除
		{
			Flash_PageErase(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE);	//擦除这个扇区
			...
			STMFLASH_Write_NoCheck_Word(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF_WORD,STM_SECTOR_SIZE/4);//写入整个扇区  
		}else 
		{
			FLASH_WaitForLastOperation(FLASH_WAITETIME);       	//等待上次操作完成
			STMFLASH_Write_NoCheck_Word(WriteAddr,pBuffer,secremain);//写已经擦除了的,直接写入扇区剩余区间. 
		}
		if(NumToWrite==secremain)break;//写入结束了
		else//写入未结束
		{
